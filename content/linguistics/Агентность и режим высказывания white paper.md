...
Исследование опирается на разметку диалогов и демонстрирует, что агентность сама по себе не является достаточным предиктором действия. Ключевым фактором оказывается режим высказывания. Предлагается формализация модели в виде [[DSL LAT|DSL]] (domain-specific language) и базовый rule-based классификатор как первый уровень анализа.
...
[[DSL LAT|DSL: Linguistic Agency Transformation (LAT)]]
